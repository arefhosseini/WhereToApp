...
public class UserActivity extends AppCompatActivity {
    ...
    private void followRequest() throws JSONException {
        GetDataService service = RetrofitClientInstance.getRetrofitInstance().create(GetDataService.class);
        Call<ResponseBody> call = service.followUser(new FollowRequest(user.getPhoneNumber(), phoneNumber).toRequestBody());
    ...
    private void removeFollowRequest() throws JSONException {
        GetDataService service = RetrofitClientInstance.getRetrofitInstance().create(GetDataService.class);
        Call<ResponseBody> call = service.removeFollowUser(new FollowRequest(user.getPhoneNumber(), phoneNumber).toRequestBody());
        call.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                //progressDoalog.dismiss();
    ...
    private View.OnClickListener onOpenFriendActivityFollowersSelected = new View.OnClickListener() {
        @Override
        public void onClick(View view) {
            Intent dashboardIntent = new Intent(UserActivity.this, FriendActivity.class);
